...
2.  **安全性**：
    ...
    - API 请求必须通过 HTTPS 发送。
3.  **UI 细节**：
    - [Login](file://react-ud\src\Login\Login.tsx) 按钮在 `isLoading` 期间应设为 `disabled`，防止重复提交。
...
这份文档已经包含了开发所需的所有核心逻辑。如果您需要，我可以基于这份设计书为您提供：

1.  **React 代码实现**：包含完整的校验逻辑和 API 模拟。
2.  **单元测试用例**：针对上述 Check 逻辑的 Jest 测试代码。
