 ...
 *
 * 测试策略：
 * - {@link ADChangeServiceImpl#findBySerieAndChnr(String, String)}：
 *   Mapper 返回实体 / 返回 null
 * - {@link ADChangeServiceImpl#insert(String, String, String, String, String)}：
 *   Mapper 返回 1（插入成功）/ 返回 0（插入失败）
 * - {@link ADChangeServiceImpl#updateAllActToN()}：
 *   Mapper 返回任意 int
 */
...
class ADChangeServiceImplTest {
    ...
    @InjectMocks
    private ADChangeServiceImpl adChangeService;
    ...
    class InsertTest {
        ...
        void shouldReturn0WhenInsertFails() {
            // Arrange
            when(adChangeMapper.insert(SERIE, CHNR, ACT, BU, REASON, "SYSTEM")).thenReturn(0);
        ...
        void shouldAcceptNullReason() {
            // Arrange
            when(adChangeMapper.insert(SERIE, CHNR, ACT, BU, null, "SYSTEM")).thenReturn(1);

            // Act
            int result = adChangeService.insert(SERIE, CHNR, ACT, BU, null);

            // Assert
            assertEquals(1, result);
            verify(adChangeMapper).insert(SERIE, CHNR, ACT, BU, null, "SYSTEM");
        }
        ...
        void shouldAcceptNullSerie() {
            // Arrange
            when(adChangeMapper.insert(null, CHNR, ACT, BU, REASON, "SYSTEM")).thenReturn(1);

            // Act
            int result = adChangeService.insert(null, CHNR, ACT, BU, REASON);

            // Assert
            assertEquals(1, result);
            verify(adChangeMapper).insert(null, CHNR, ACT, BU, REASON, "SYSTEM");
        }
        ...
        void shouldAcceptNullChnr() {
            // Arrange
            when(adChangeMapper.insert(SERIE, null, ACT, BU, REASON, "SYSTEM")).thenReturn(1);

            // Act
            int result = adChangeService.insert(SERIE, null, ACT, BU, REASON);

            // Assert
            assertEquals(1, result);
            verify(adChangeMapper).insert(SERIE, null, ACT, BU, REASON, "SYSTEM");
        }
    }
    ...
    class UpdateAllActToNTest {
        ...
        void shouldReturnUpdateCountWhenGreaterThanZero() {
            // Arrange
            when(adChangeMapper.updateAllActToN()).thenReturn(5);

            // Act
            int result = adChangeService.updateAllActToN();

            // Assert
            assertEquals(5, result);
            verify(adChangeMapper).updateAllActToN();
        }
        ...
        void shouldReturnZeroWhenNoRecordsUpdated() {
            // Arrange
            when(adChangeMapper.updateAllActToN()).thenReturn(0);
