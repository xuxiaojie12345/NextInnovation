...
public class UD08HomologationVariablesController {
    ...
    private static final Logger log = LoggerFactory.getLogger(UD08HomologationVariablesController.class);
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> checkVariable(@RequestBody Map<String, String> request) {
        try {
            String variable = request.get("variable");
            if (variable == null || variable.trim().isEmpty()) {
                return ResponseEntity.badRequest()
                    .body(ApiResponse.error(400, "Variable is required."));
            }
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> checkRule(@RequestBody Map<String, String> request) {
        try {
    ...
    @PostMapping("/add")
    public ResponseEntity<ApiResponse<Map<String, String>>> addRule(@RequestBody HdocUserDefinedRules rule) {
        try {
            ...
            if (result > 0) {
                ...
                return ResponseEntity.status(500)
                    .body(ApiResponse.error(500, "Failed to add rule."));
            }
        } catch (Exception e) {
    ...
    @PostMapping("/update")
    public ResponseEntity<ApiResponse<Map<String, Object>>> updateRule(@RequestBody HdocUserDefinedRules rule) {
        try {
            if (rule.getPc() == null || rule.getNum() == null || rule.getMarket() == null) {
                return ResponseEntity.badRequest()
                    .body(ApiResponse.error(400, "pc, num and market are required."));
    ...
    @PostMapping("/delete")
    public ResponseEntity<ApiResponse<Map<String, Object>>> deleteRule(@RequestBody Map<String, String> request) {
        try {
            String pc = request.get("pc");
            String num = request.get("num");
            String market = request.get("market");
