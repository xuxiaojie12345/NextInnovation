...
public class UD09Controller {
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> search(
        ...
        try {
            ...
            List<Map<String, String>> conditions =
                (List<Map<String, String>>) request.get("conditions");
            ...
            if (conditions != null) {
                for (Map<String, String> cond : conditions) {
                    ...
                    switch (label) {
                        case "Product class":
                            if (rawVal != null && !rawVal.isEmpty()) { pc = rawVal; pcOp1 = op; }
                            break;
                        case "Number":
                            if (rawVal != null && !rawVal.isEmpty()) { num = rawVal; numOp1 = op; }
                            break;
                        case "Market":
                            if (rawVal != null && !rawVal.isEmpty()) { market = rawVal; marketOp1 = op; }
                            break;
                        case "Variable":
                            if (rawVal != null && !rawVal.isEmpty()) { variable = rawVal; variableOp1 = op; }
                            break;
                        case "Value":
                            if (rawVal != null && !rawVal.isEmpty()) { valField = rawVal; valOp1 = op; }
                            break;
                        case "Variant string":
                            if (v1 != null && !v1.isEmpty()) { vs = v1; vsOp1 = op1; }
                            if (v2 != null && !v2.isEmpty()) { vsVal2 = v2; vsOp2 = op2; }
                            break;
                        case "Comments":
                            if (rawVal != null && !rawVal.isEmpty()) { comments = rawVal; commentsOp1 = op; }
                            break;
                        case "Add":
                            if (rawVal != null && !rawVal.isEmpty()) { addDate = rawVal; addDateOp1 = op; }
                            break;
                        case "Delete":
                            if (rawVal != null && !rawVal.isEmpty()) { deleteDate = rawVal; deleteDateOp1 = op; }
                            break;
                        case "Created by user":
                            if (rawVal != null && !rawVal.isEmpty()) { createdBy = rawVal; createdByOp1 = op; }
                            break;
                        case "Date":
                            if (rawVal != null && !rawVal.isEmpty()) { date = rawVal; dateOp1 = op; }
                            break;
                        default: break;
                    }
                }
            ...
            List<Map<String, Object>> list = service.searchRules(
                pc, num, market, variable, valField,
                pcOp1, numOp1, marketOp1, variableOp1, valOp1,
                vs, vsOp1, vsOp2, vsVal2,
                comments, commentsOp1,
                addDate, addDateOp1,
                deleteDate, deleteDateOp1,
                createdBy, createdByOp1,
                date, dateOp1);
            ...
            for (Map<String, Object> row : list) {
                ...
                item.put("updateDatetime", row.get("REGISTER_DATETIME") != null ?
                    row.get("REGISTER_DATETIME").toString() : null);
                ruleList.add(item);
            }
    ...
    @RequestMapping(value = "/deleteSelected", method = {RequestMethod.POST, RequestMethod.DELETE})
        ...
        try {
            ...
            if (records == null || records.isEmpty()) {
                return ResponseEntity.badRequest()
                    .body(ApiResponse.error(400, "No records to delete."));
            }
