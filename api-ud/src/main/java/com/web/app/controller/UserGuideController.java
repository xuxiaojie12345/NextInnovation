...
public class UserGuideController {
    ...
    /**
     * 获取 User Guide 帮助链接列表（基于权限过滤）
     * GET /api/v1/hdoc/user-guide/links
     */
    ...
    public ResponseEntity<ApiResponse<List<UserGuideLink>>> getHelpLinks() {
        try {
            ...
            return ResponseEntity.status(500)
                .body(ApiResponse.error(500, "无法加载帮助菜单，请稍后重试"));
        }
    }

    /**
     * 获取用户权限（用于前端控制链接显示）
     * GET /api/v1/hdoc/user/permissions
     */
    ...
    public ResponseEntity<ApiResponse<UserPermissions>> getUserPermissions() {
        try {
            ...
            return ResponseEntity.status(500)
                .body(ApiResponse.error(500, "权限服务不可用"));
        }
    }
    ...
    private List<UserGuideLink> getDefaultHelpLinks() {
        ...
        links.add(createLink("HDoc - Market Document Setting", "/menu/market-document-setting", null, true));
    ...
    public static class UserPermissions {
        ...
        public boolean isCanViewTemplates() { return canViewTemplates; }
        public void setCanViewTemplates(boolean canViewTemplates) { this.canViewTemplates = canViewTemplates; }

        public boolean isCanEditVariables() { return canEditVariables; }
        public void setCanEditVariables(boolean canEditVariables) { this.canEditVariables = canEditVariables; }

        public boolean isCanViewMarkets() { return canViewMarkets; }
        public void setCanViewMarkets(boolean canViewMarkets) { this.canViewMarkets = canViewMarkets; }

        public boolean isCanViewDocumentSettings() { return canViewDocumentSettings; }
        public void setCanViewDocumentSettings(boolean canViewDocumentSettings) { this.canViewDocumentSettings = canViewDocumentSettings; }
    }
}
