...
public class TemplateController {
    ...
    public ResponseEntity<ApiResponse<Object>> uploadFile(
        ...
        try {
            ...
            if (market == null || market.trim().isEmpty()) {
                return ResponseEntity.badRequest()
                    .body(ApiResponse.error(400, "Market is required."));
            }
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> listTemplates(@RequestBody Map<String, String> request) {
        try {
            String market = request.get("market");
            if (market == null || market.trim().isEmpty()) {
                return ResponseEntity.badRequest()
            ...
            if (dir.exists() && dir.isDirectory()) {
                templates = Arrays.stream(Objects.requireNonNull(dir.listFiles()))
                    .filter(File::isFile)
                    .map(file -> {
    ...
    public ResponseEntity<?> downloadFile(@RequestBody Map<String, String> request) {
        try {
            ...
            return ResponseEntity.status(500)
                .body(ApiResponse.error(500, "File not found."));
        }
    }
}
