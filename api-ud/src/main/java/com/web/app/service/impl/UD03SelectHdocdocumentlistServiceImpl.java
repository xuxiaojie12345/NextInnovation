...
public class UD03SelectHdocdocumentlistServiceImpl implements UD03SelectHdocdocumentlistService {
    ...
    public ApiResponse<UD03SelectHdocdocumentlistResponse> selectHdocdocumentlist() {
        log.info("========== UD03 Select Hdocdocumentlist Start ==========");
        ...
        try {
            ...
            if (doctypeList == null || doctypeList.isEmpty()) {
                log.warn("No document types found in HDOC_DOCUMENT_LIST table");
            ...
            // 4.7 记录查询日志
            log.info("Query successful - Found {} document types", doctypeList.size());
            log.info("Document types: {}", doctypeList);
            ...
            // 返回成功响应
            log.info("========== UD03 Select Hdocdocumentlist End ==========");
            return ApiResponse.success(response);
            
        } catch (Exception e) {
            log.error("System error occurred while querying document list", e);
